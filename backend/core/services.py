...
def current_net_worth():
    ...
    portfolio + bank, not bank + saxo_account_value: Saxo's own cash is
    already mirrored into a BankAccount row (saxo.mapping.SAXO_CASH_ACCOUNT_ID),
    written atomically alongside the same balance response that produces
    saxo_account_value, so bank already includes it - adding saxo_account_value
    on top would double-count that cash. This also stays trade-neutral for a
    BUY/SELL: a sale drops portfolio_value and raises the Saxo-cash-mirror
    balance (part of bank) by the same amount, in the same sync.
