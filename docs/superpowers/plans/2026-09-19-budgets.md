...
```python
# In backend/enablebanking/test_transaction_views.py, change the existing:
#   from .models import BankTransaction, Subscription
# to:
#   from .models import BankTransaction, Budget, Subscription
# then append below the existing test classes:
...
`Decimal`, `date`, and `BankAccount` are already imported at the top of this file (from Task 10's `SpendingSummaryViewTest` and Task 17's `SpendingTrendViewTest`) — no other import changes needed beyond the `.models` line above.
